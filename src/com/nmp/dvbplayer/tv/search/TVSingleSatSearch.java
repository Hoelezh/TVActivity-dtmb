...
public class TVSingleSatSearch extends TVSearch {
	...
	protected void initParams() {
		...
		name = res.getString(R.string.tv_search_scantype);
		this.mScantype = new TVStringParam(name);
		name = res.getString(R.string.tv_search_preset);
		this.mScantype.addValue(name);
		name = res.getString(R.string.tv_search_auto_scan);
		this.mScantype.addValue(name);
		name = res.getString(R.string.tv_search_accurate);
		this.mScantype.addValue(name);
		this.mScantype.changeIdx(0);
		this.addParam(this.mScantype);
	...
	public DvbSearchParam getSearchParams() {
		...
		if(this.mScantype.getIndex()==0)
			return new DvbSearchParam(freq, freq, bandwidth, sym, por, nit,0x00/*dvbs*/,1,fta,protype);
		else if(this.mScantype.getIndex()==1)
			return new DvbSearchParam(freq, freq, bandwidth, sym, por, nit,0x00/*dvbs*/,2,fta,protype);
		else
			return new DvbSearchParam(freq, freq, bandwidth, sym, por, nit,0x00/*dvbs*/,3,fta,protype);
	}
